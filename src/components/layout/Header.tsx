...
export function Header() {
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6 lg:px-8 py-4">
      ...
      <Button variant="outline" size="icon" className="ml-auto h-8 w-8" onClick={() => alert('Notifications clicked!')}>
        <Bell className="h-4 w-4" />
        <span className="sr-only">Toggle notifications</span>
      </Button>
      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => alert('User profile clicked!')}>
